...
public class PoseReceiver : MonoBehaviour
{
    ...
    Vector3 ConvertMediaPipeToUnity(Vector3 landmark, float baseDepth = 0.0f)
    {
        ...
        float unityZ = baseDepth + (-landmark.z * 0.85f); // Convert negative depth (1.7/2 = 0.85)
    ...
    void LateUpdate()
    {
        ...
        leftFootTarget.position = Vector3.MoveTowards(leftFootTarget.position, ConvertMediaPipeToUnity(receivedPose.LEFT_ANKLE), Time.deltaTime * 5);
        rightFootTarget.position = Vector3.MoveTowards(rightFootTarget.position, ConvertMediaPipeToUnity(receivedPose.RIGHT_ANKLE), Time.deltaTime * 5);
